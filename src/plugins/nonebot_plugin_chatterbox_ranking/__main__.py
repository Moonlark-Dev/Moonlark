...
summary = on_alconna(Alconna("chatterbox", Subcommand("--enable|-e"), Subcommand("--disable|-d"), Args["user_id?", str | At]), aliases={"ct"})
...
async def _(session: async_scoped_session, user_id: str, sender_id: str = get_user_id(), group_id: str = get_group_id()) -> None:
    if group_id not in await get_config():
        await lang.finish("disabled", user_id)
    if user_id == "me":
        user_id = sender_id
