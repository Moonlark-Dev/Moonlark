...
@on_alconna(Alconna("t", Args["text", MultiVar(str)], Option("-s|--source", Args["source_lang", str, "en"]), Option("-t|--target", Args["target_lang", str, "zh"]))).handle()
async def _(text: list[str], source_lang: str, target_lang: str, matcher: Matcher, user_id: str = get_user_id()) -> None:
...
async def _(text: str, matcher: Matcher, user_id: str = get_user_id()) -> None:
    result_str = (await network.translate(" ".join([(await network.translate(char, "zh", "en", user_id)).data for char in " ".join(text)]), "en", "zh", user_id)).data
    if not (c := await review_text(result_str))["compliance"]:
        await lang.finish("review_unpassed", user_id, c["message"])
    await matcher.finish(result_str, reply_message=True)
