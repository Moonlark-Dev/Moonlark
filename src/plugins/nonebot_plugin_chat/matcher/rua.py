...
alc = Alconna(
    "rua",
    Subcommand("action", Args["target_index", int])
)
...
RUA_ACTIONS: dict[int, RuaAction] = {
    1: RuaAction(
        name="poke",
        refusable=True,
        unlock_favorability=0.0
    ),
    2: RuaAction(
        name="head",
        refusable=True,
        unlock_favorability=0.007
    ),
    3: RuaAction(
        name="chin",
        refusable=False,
        unlock_favorability=0.051
    ),
    4: RuaAction(
        name="cheek",
        refusable=False,
    ...
    5: RuaAction(
        name="hug",
        refusable=False,
        unlock_favorability=0.151
    ),
    6: RuaAction(
        name="ears",
        refusable=True,
        unlock_favorability=0.300
    )
}
...
async def _(user_id: str = get_user_id()) -> None:
    ...
    for action_id, action in RUA_ACTIONS.items():
        ...
        if action["name"] == selected_action["name"]:
            actions.append(await lang.text("rua.action_list_item", user_id, "*", await lang.text(f"rua.actions.{action['name']}.name", user_id)))
        else:
            actions.append(await lang.text("rua.action_list_item", user_id, action_id, await lang.text(f"rua.actions.{action['name']}.name", user_id)))
    await lang.finish("rua.action_list", user_id, "\n".join(actions))

@matcher.assign("$main")
async def _(bot: Bot, event: Event, target: Target, group_id: str = get_group_id(), user_id: str = get_user_id()) -> None:
