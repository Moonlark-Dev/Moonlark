...
class MoonlarkMain:
    ...
    async def _generate_system_prompt(self, summary: str, state_info: dict, suggestions: str) -> str:
        ...
        blog_text = (
            f"草稿《{draft['topic']}》，{draft['word_count']}字，续写{draft['continue_count']}次" if draft else "无草稿"
        )

        proactive_info = state_info.get("proactive_info", {})
        if proactive_info:
            parts = [
                f"{uid}: {info.get('last_chat', '').strftime('%H:%M')}"
                for uid, info in proactive_info.items()
                if info.get("last_chat")
            ]
    ...
    async def _execute_decision(self, decision: EgoDecisionResponse) -> None:
        ...
        # start_new_topic → BlogWriter 开始新博客
        if decision.start_new_topic:
            await self.blog_writer.handle_action(f"start_new_topic:{decision.start_new_topic}")
            actions_taken.append(f"blog: start_new_topic:{decision.start_new_topic}")
    ...
    async def submit_sleep_decision(
        ...
        try:
            result = await self.sleep_controller.submit_sleep_decision(deal_type, delay_minutes or 5, reason or "")
            if future and not future.done():
    ...
    async def _get_additional_prompt(self) -> str:
        ...
        return await lang.text(
            ...
            state_str,
            (
                "\n".join([await self._format_note(n) for n in notes])
                if notes
