...
from ...utils.status_manager import StatusManager, get_status_manager
...
class MoonlarkMain:
    ...
    def _collect_state(self) -> dict:
        """收集当前状态（mood 直接从 StatusManager 获取）"""
    ...
    async def _execute_decision(self, response: dict) -> None:
        ...
        if self_action and self_action != "nothing":
            await self.self_action.start_activity(self_action)
    ...
    def on_message_received(self) -> None:
        """收到消息时调用，更新 SleepController 的 last_message_time"""
        self.sleep_controller.handle_message()

    def on_reply_sent(self) -> None:
        """发送回复时调用，更新 SleepController 的 last_reply_time"""
