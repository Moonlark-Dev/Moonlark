...
class Interrupter:
    ...
    def _check_random_interrupt(self, user_id: str) -> bool:
        ...
        logger.debug(f"Random interrupt check for user {user_id}: {'Interrupted' if should_interrupt else 'Not interrupted'}")
    ...
    def _check_duplicate_content(self, message: str) -> bool:
        ...
        for cached_msg in recent_messages:
            if cached_msg["self"]:  # 只检查机器人发送的消息
                ...
                if len(cached_keywords) > 0 and len(message_keywords.intersection(cached_keywords)) / len(cached_keywords) > 0.5:
                    return True
    ...
    def _check_frequency_interrupt(self) -> bool:
        ...
        if long_term_count > 20:
            ...
            logger.debug(f"Long-term frequency limit exceeded ({long_term_count} messages in 60 seconds), responding with 10% probability: {'Yes' if should_respond else 'No'}")
    ...
    def _check_time_related_interrupt(self) -> bool:
        ...
        if 2 <= current_hour < 6:
            ...
            logger.debug(f"Low activity period (hour {current_hour}), interrupting with 50% probability: {'Yes' if should_interrupt else 'No'}")
            return should_interrupt
