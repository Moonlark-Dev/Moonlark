...
class NoteManager:
    ...
    async def get_note_by_id(self, note_id: int) -> Optional[Note]:
        ...
        async with get_session() as session:
            query = select(Note).where(
                Note.id == note_id,
                Note.context_id == self.context_id
            )
            result = await session.scalars(query)
            return result.first()

    async def update_note(self, note_id: int, content: Optional[str] = None, 
                         keywords: Optional[str] = None, expire_days: Optional[int] = None) -> bool:
        """
        Update a note
    ...
    async def filter_note(self, chat_history: str, memory_topics: list[str], include_expired: bool = False) -> List[Note]:
        """
