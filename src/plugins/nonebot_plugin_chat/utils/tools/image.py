...
class ImageDownloadError(ImageFormatError):
...
def _normalize_image(data: bytes) -> tuple[bytes, str]:
    ...
    统一委托给 :mod:`nonebot_plugin_chat.utils.image_format`，保证 URL 下载与
    聊天消息中的图片使用同一套格式校验与转换规则。
    ...
    Raises:
        ImageDownloadError: 内容不是可识别的图片或格式无法转换
    """
    try:
