...
class MemoryActivator:
    ...
    async def activate_memories_from_text(
        self, context_id: str, target_message: str, chat_history: str = "", max_memories: int = 5
    ) -> List[Tuple[str, str]]:
        """
        从文本中激活相关记忆
                    ...
                    candidate_memories.append(
                        {
                            ...
                            "relevance": self._calculate_relevance(keyword, target_message, memory_content),
                        }
                    )
        ...
        # 使用LLM选择最相关的记忆
        selected_memories = await self._select_memories_with_llm(
            target_message, chat_history, candidate_memories, max_memories
        )

        return selected_memories

    def _calculate_relevance(self, keyword: str, target_message: str, memory_content: str) -> float:
        """计算记忆相关性（简化版）"""
    ...
    async def _select_memories_with_llm(
        self, target_message: str, chat_history: str, candidate_memories: List[Dict[str, Any]], max_memories: int
    ) -> List[Tuple[str, str]]:
        """使用LLM选择最相关的记忆"""
        try:
            # 构建记忆信息
