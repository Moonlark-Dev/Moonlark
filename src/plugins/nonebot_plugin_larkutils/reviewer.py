...
async def get_access_token(force_update=False):
    ...
    async with aiofiles.open(plugin_cache_dir.joinpath("baiduAPI.json"), "w", encoding="utf-8") as f:
        await f.write(json.dumps({"access_token": access_token, "expires_in": int(time.time() - 30) + expires_in}))
    return access_token
...
def get_review_result(data: dict) -> ReviewResult:
    ...
    try:
        ...
        result: ReviewResult = {"compliance": False, "conclusion": "出错", "message": data.get("error_msg")}
