    ...
    try:
        ...
        if await ask_cave_submission(user_id):
            ...
            try:
                ...
                await lang.finish("cave_image_prompt.error", user_id, f"内容审核失败: {e.reason}")
            except EmptyImage:
                await lang.finish("cave_image_prompt.error", user_id, "图片获取失败")
            except DuplicateCave as e:
                # 构建相似洞穴信息
                async with async_scoped_session() as session:
                    from nonebot_plugin_alconna import Text
                    msg = UniMessage(await lang.text("cave_image_prompt.error", user_id, "发现相似的回声洞"))
