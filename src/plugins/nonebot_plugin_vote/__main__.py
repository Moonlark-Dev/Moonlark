...
from .utils import create_vote, generate_vote_image, generate_vote_list, get_vote_data, get_choice_content, is_user_voted, is_vote_open
...
async def _(
    ...
    if result.find("create"):
        ...
        await vote.finish(UniMessage().image(raw=await generate_vote_list(
            user_id,
            group_id,
            session,
            result.find("all")
...
async def _(
    ...
    if choice.lower() in ["ok", "save", "done"]:
        ...
        await vote.reject(prompt=await lang.text(
            "create.choice_deleted",
            user_id,
            state["choices"].pop(-1)
        ))
    else:
        ...
        await vote.reject(await lang.text(
            "create.get_choice",
            user_id,
            len(state["choices"])
        ))
