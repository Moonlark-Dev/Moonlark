...
async def get_schedule_status(user_id: str, schedule_id: str, session: async_scoped_session | AsyncSession) -> Optional[UserSchedule]:
    return await session.scalar(select(UserSchedule).where(
