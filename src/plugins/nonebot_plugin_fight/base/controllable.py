...
class ControllableRoundBoundary(Monomer):
    ...
    async def on_action(self, teams: list[Team]) -> None:
        ...
        template_body = {
            "monomers_current_team": [
                {
                    ...
                    "skills": [{
                        "index": i + 1,
                        "name": skill["name"],
                        "info": await self.get_skill_info(skill)
                    } for i, skill in enumerate(skills) if skill["monomer"] == monomer]
            ...
            "monomers_another_team": [{
                ...
                "name": await monomer.get_name(self.user_id),
                "stat": await self.team.get_monomer_stat(monomer)
            } for monomer in teams[0].get_monomers() if monomer.is_selectable()],
            "lang": {
        ...
        image = await render_template(
            "fight_skill_list.html.jinja",
            await lang.text("action.title", self.user_id),
            self.user_id,
            template_body
        )
        ...
        while True:
            waiter = WaitUserInput(
                message,
                self.user_id,
                lambda string: re.match(r"^[0-9a-z \.]+$", string) is not None and parse_action_command(string, skills, monomers, teams[0]) is not None
            )
            await waiter.wait()
    ...
    def process_commands(self, commands: list[ActionCommand]) -> None:
        for command in commands:
            ...
            if not command["skill_info"]["instant"]:
                ...
                command["skill_info"]["monomer"].tigger_command_after(command, commands[commands.index(command) - 1] if commands.index(command) > 0 else None)
    ...
    async def get_sorted_momoners_stat(self) -> list[dict[str, Any]]:
        ...
        return [
            ...
            for monomer in self.team.scheduler.get_sorted_monomers() if not isinstance(monomer, ControllableRoundBoundary)
        ]
    ...
    def check_action_commands(self, commands: list[ActionCommand], monomers: list[ControllableMonomer], enemy_team: Team) -> bool:
        ...
        for command in commands:
            if command["skill_info"]["instant"] or not command["skill_info"]["occupy_round"]:
                continue
            monomer_action_count[command["skill_info"]["monomer"]] = 1 + monomer_action_count.get(command["skill_info"]["monomer"], 0)
        ...
        for monomer in monomers:
            for command in [cmd for cmd in commands if cmd["skill_info"]["monomer"] == monomer and not cmd["skill_info"]["instant"]]:
                points -= command["skill_info"]["cost"]
                if points < 0: return False
...
def parse_action_command(string: str, skills: list[SkillInfo], monomers: list[ControllableMonomer], enemy_team: Team) -> Optional[list[ActionCommand]]:
    ...
    while len(unparsed_commands) == 0:
        ...
        if skill["target_type"] != "none" and target_index is None and len(unparsed_commands) > 0 and unparsed_commands[0].isdigit():
        ...
        commands.append({
            "skill_index": skill_index - offset,
            "target": target,
            "skill_info": skill
        })
    return commands