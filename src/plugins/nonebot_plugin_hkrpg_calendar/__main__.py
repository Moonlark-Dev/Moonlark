...
async def get_calendar() -> EventListDict:
    ...
    async with httpx.AsyncClient() as client:
        request = await client.get("https://raw.githubusercontent.com/Trrrrw/hoyo_calendar/refs/heads/main/ics/%E5%B4%A9%E5%9D%8F%EF%BC%9A%E6%98%9F%E7%A9%B9%E9%93%81%E9%81%93.ics")
...
async def _(user_id: str = get_user_id()) -> None:
    ...
    await UniMessage().image(raw=await render_template(
        "hkrpg_calendar.html.jinja",
        await lang.text("title", user_id),
        user_id,
        {"events": events},
        await generate_render_keys(lang, user_id, [f"template.{k}" for k in ["ongoing", "remain", "day", "coming", "coming_day"]])
    )).send()
