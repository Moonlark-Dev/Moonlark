...
async def get_durations(user_id: str = get_user_id()) -> DurationsResponse:
    ...
    if len(durations.data.projects) <= 0:
        durations.data.projects = [DurationsProject(
            name=await lang.text("main.none", user_id),
            text=await lang.text("main.zero", user_id)
        )]
    return durations
    ...
    image = await render_template(
        ...
        user_id,
        {
            ...
            "project": await lang.text(
                "info.project",
                user_id,
                durations.data.projects[0].name,
                durations.data.projects[0].text
            )
        }
    )
