...
async def divorce(group_id: str, session: async_scoped_session, platform_user_id: str) -> None:
    ...
    query = cast(
        ...
        await session.scalar(
            select(WifeData).where(
                WifeData.user_id == platform_user_id, WifeData.group_id == group_id, WifeData.generate_date == today
            )
        ),
    )
    ...
    if query:
        ...
        result = await session.scalar(
            select(WifeData).where(
                WifeData.user_id == query.wife_id, WifeData.group_id == group_id, WifeData.generate_date == today
            )
        )
        if result:
